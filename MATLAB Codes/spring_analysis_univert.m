...
d  =  dir2cell(dir([p1,'*Spring*']),true); % Directory of samples

% Load in running results compilation, formatted as a table
if exist('spring_results.mat','file')
...
for ii = 1:L
   ...
   [~,mx]       = max  (force    );
    bumper      = round(mx * 0.25);
    disp_stiff  = disp( 1+bumper:mx-bumper);
    force_stiff = force(1+bumper:mx-bumper);
    time_stiff  = time( 1+bumper:mx-bumper);
    ...
    print(['Spring Calibration\stiffness_',d{ii},'.tif'],'-dtiff','-r150');
    close(gcf);
    ...
    scatter(time(mx),disp(mx),'*','MarkerEdgeColor','b');
    subplot(2,1,2); plot(time,force,'LineWidth',1,'Color','r');
    xlabel('Time (s)'); ylabel('Load (N)');
    hold on; plot([0.15,0.15],get(gca,'ylim'),...
        'LineWidth',1,'Color','k','LineStyle','--');
    scatter(time(mx),force(mx),'*','MarkerEdgeColor','r');
    print(['Spring Calibration\ramp_',d{ii},'.tif'],'-dtiff','-r150');
    close(gcf);
    ...
    print(['Spring Calibration\precond_',d{ii},'.tif'],'-dtiff','-r150');
    close(gcf);
    ...
    results_current = cell2table({d{ii},max_load,max_disp,total_disp,...
...
last_update = date;
save('spring_results.mat','results_all','last_update');
writetable(results_all,'spring_results.csv');